...
// Spawn an enemy with increasing difficulty and a bullet cooldown.
function spawnEnemy() {
  const enemy = {
    ...
    health: 30 + wave * 5,
    bulletCooldown: Math.random() * 2000 + 1000 // enemy fires between 1-3 sec intervals
  };
  enemies.push(enemy);
}
...
// Enemy shoots a bullet (from enemy's current position)
...
function update() {
  ...
  enemies.forEach((enemy, index) => {
    ...
    enemy.bulletCooldown -= 16; // approximate frame decrement
    if (enemy.bulletCooldown <= 0) {
      enemyShoot(enemy);
      enemy.bulletCooldown = Math.random() * 2000 + 1000; // reset cooldown
    }
  ...
  enemyBullets.forEach((bullet, index) => {
    ...
    if (isColliding(bullet, player)) {
      if (!player.shieldActive) player.health -= 15;
  ...
  // Update power-ups (update lifetime, remove expired, check for collection)
  ...
  // Draw power-ups with countdown timer and type text
  powerUps.forEach(powerUp => {
    ...
    ctx.fillText(powerUp.type + " " + seconds + "s", powerUp.x + 2, powerUp.y + powerUp.height / 2);
  });
...
function survivalStartGame() {
  ...
  player.speed = player.baseSpeed;
  // Clear enemies, enemy bullets, and reset wave
  enemies.length = 0;
